...
- (void)draw
{
    ...
    if (_MAType == Y_BOLL_DN || _MAType == Y_BOLL_MB || _MAType == Y_BOLL_UP){
       ...
       // NSAssert(!isnan(firstPoint.x) && !isnan(firstPoint.y), Localized(@"出现NAN值：BOLL画线", nil));
...
//        UIColor *lineColor = self.MAType == Y_MA5Type ? [UIColor ma5Color] : (self.MAType == Y_MA30Type ? [UIColor ma30Color] : [UIColor mainTextColor]);
        ...
        //NSAssert(!isnan(firstPoint.x) && !isnan(firstPoint.y), Localized(@"出现NAN值：MA画线", nil));
        CGContextMoveToPoint(self.context, firstPoint.x, firstPoint.y);

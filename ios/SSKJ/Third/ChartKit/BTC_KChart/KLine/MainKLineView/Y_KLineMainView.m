...
//    CGContextFillRect(context, CGRectMake(0, Y_StockChartKLineMainViewMaxY, self.frame.size.width, self.frame.size.height - Y_StockChartKLineMainViewMaxY));
    ...
    if(self.MainViewType == Y_StockChartcenterViewTypeKline)
    {
        ...
        [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (idx == 0) {
                self.parentScrollView.contentOffset = CGPointMake(positionModel.ClosePoint.x, 0);
            }
        }];
...
- (void)drawMainView{
   // NSAssert(self.kLineModels, Localized(@"kLineModels不能为空", nil));
...
//    [self clearsContextBeforeDrawing];
    ...
    //    if (offset > 0)
    //    {
    //        //NSLog(Localized(@"计算出来的位移%f", nil),offset);
 ...
 */
- (CGFloat)getExactYPositionWithOriginXPosition:(CGFloat)originXPosition{
    CGFloat xPositoinInMainView = originXPosition;
    NSInteger startIndex = (NSInteger)((xPositoinInMainView - self.startXPosition) / ([Y_StockChartGlobalVariable kLineGap] + [Y_StockChartGlobalVariable kLineWidth]));
    NSInteger arrCount = self.needDrawKLinePositionModels.count;
    for (NSInteger index = startIndex > 0 ? startIndex - 1 : 0; index < arrCount; ++index) {
        ...
        if(xPositoinInMainView > minX && xPositoinInMainView < maxX)
        {
            ...
            return kLinePositionModel.ClosePoint.y + 20;
        }
...
//    //NSLog(Localized(@"这是模型开始的index-----------%lu", nil),needDrawKLineStartIndex);
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels {
    ...
    CGFloat maxY = self.parentScrollView.frame.size.height * [Y_StockChartGlobalVariable kLineMainViewRadio];
...
//        }
        ...
        CGFloat xPosition = self.startXPosition + idx * (lineWidth + lineGap);
...
//        }
       ...
       // NSAssert(!isnan(ma5Y) && !isnan(ma10Y) && !isnan(ma20Y), Localized(@"出现NAN值", nil));
        ...
        if(_targetLineStatus == Y_StockChartTargetLineStatusBOLL){
           ...
           ////NSLog(Localized(@"position：\n上: %@ \n中: %@ \n下: %@", nil),kLineModel.BOLL_UP,kLineModel.BOLL_MB,kLineModel.BOLL_DN);
           ...
           // NSAssert(!isnan(boll_mbY) && !isnan(boll_upY) && !isnan(boll_dnY), Localized(@"出现BOLL值", nil));
...
-(void)parentDidScrollScrollView:(UIScrollView *)scrollView
{

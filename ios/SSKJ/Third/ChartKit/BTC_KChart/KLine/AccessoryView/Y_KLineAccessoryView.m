...
- (void)draw
{
    ...
    //NSAssert(self.needDrawKLineModels && self.needDrawKLinePositionModels && self.kLineColors && kLineColorCount == kLineModelcount && kLinePositionModelCount == kLineModelcount, /*Localized(@"数据异常，无法绘制Volume", nil)*/);
...
//                    DEAY = maxY - (model.DEA.floatValue - minValue)/unitValue;
           ...
           // NSAssert(!isnan(DIFY) && !isnan(DEAY), Localized(@"出现NAN值", nil));
        ...
        [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
           ...
           // NSAssert(!isnan(KDJ_K_Y) && !isnan(KDJ_D_Y) && !isnan(KDJ_J_Y), Localized(@"出现NAN值", nil));

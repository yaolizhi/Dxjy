...
{
   // NSAssert(arr.count == 6, Localized(@"数组长度不足", nil));

    if (self)
    {
        NSInteger pointNumber = [Y_StockChartGlobalVariable decimalPoint];
        
        NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
        formatter.roundingMode = NSNumberFormatterRoundFloor;
        formatter.maximumFractionDigits = 4;  //保留最多两位小数（99.00->99;99.01->99.01;99.10->99.1）
        //    //NSLog(Localized(@"字符串最多保留两位小数且四舍不入--%@", nil), [formatter stringFromNumber:number]); // 99.99
        //    //NSLog(Localized(@"字符串保留两位小数且四舍五入--%.2lf", nil), num); // --100.00
//        NSNumber *closeNumber = [formatter numberFromString:dic[@"price"]];

        
        
        _Date = arr[0];
//        _Open = @([arr[1] floatValue]);
        _Open = [formatter numberFromString:arr[1]];
//        _High = @([arr[2] floatValue]);
        _High = [formatter numberFromString:arr[2]];
//        _Low = @([arr[3] floatValue]);
        _Low = [formatter numberFromString:arr[3]];
//        _Close = @([arr[4] floatValue]);
        _Close = [formatter numberFromString:arr[4]];

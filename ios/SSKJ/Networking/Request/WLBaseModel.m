...
+ (NSString *)jsonStringWithObjectArray:(NSArray *)objectArray {
    ...
    if (error) {
        SsLog(@"Error occured when create json string with object array: %@, error: %@", objectArray, error.localizedDescription);
    }
...
- (id)initWithJsonString:(NSString *)json {
    ...
    if (self) {
        ...
        if (error) {
            SsLog(@"Error occured when init object with json string: %@, error: %@", json, error.localizedDescription);
            return nil;
        }
...
- (NSString *)jsonStringRepresentation
{
    ...
    if (error)
    {
        SsLog(@"Error occured when create json string representation, error: %@", error.localizedDescription);
    }

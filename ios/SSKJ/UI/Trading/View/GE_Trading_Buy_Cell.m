...
@interface GE_Trading_Buy_Cell()<UITextFieldDelegate>
...
@property (nonatomic,strong) GE_Trading_ChooseGoods_View *chooseGoodsView;

@property (nonatomic,strong) GE_Trading_OneStock_Model *oneStockModel1;//单只股票详情

@end

@implementation GE_Trading_Buy_Cell
...
- (UILabel *)nowPriceLabel{
    if (_nowPriceLabel == nil) {
        _nowPriceLabel = [FactoryUI createLabelWithFrame:CGRectZero text:@"----" textColor:RED_HEX_COLOR font:systemFont(15)];
...
- (UITextField *)priceTextField{
    if (_priceTextField == nil) {
        _priceTextField = [FactoryUI createTextFieldWithFrame:CGRectZero text:@"以市场上最优价融资" placeHolder:@""];
...
- (UILabel *)baozhengjinLabel{
    if (_baozhengjinLabel == nil) {
        _baozhengjinLabel = [FactoryUI createLabelWithFrame:CGRectZero text:@"0" textColor:kMainBlackColor font:systemFont(15)];
...
- (UILabel *)totalPriceLabel{
    if (_totalPriceLabel == nil) {
        ...
        [_totalPriceLabel setHidden:YES];
        [_totalPriceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(@(-15));
            make.top.equalTo(self.totalPriceTitle.mas_top);
            make.height.equalTo(@40);
        }];
    }
...
- (UIButton *)sureBtn{
    if (_sureBtn == nil) {
        _sureBtn = [FactoryUI createButtonWithFrame:CGRectZero title:@"做多" titleColor:kMainWihteColor imageName:nil backgroundImageName:nil target:self selector:@selector(sureBtnAction) font:systemFont(15)];
        _sureBtn.backgroundColor = RED_HEX_COLOR;
...
-(void)currentMoney
{
    ...
    //手购买数量*现价/杠杆
    amount = buyNum*self.nowPriceLabel.text.doubleValue*self.oneStockModel1.leverage_rz.doubleValue;
    //综合服务费
...
//        [WLTools noroundingStringWith:fee afterPointNumber:6];
    }
}
...
- (void)sureBtnAction
{
    ...
    else
    {
        ...
        if (self.BuyOrderBlock) {
            NSDictionary *dic = @{@"otype":@"1",

//
//  GE_Trading_Sell_ViewController.m
//  SSKJ
//
...
@interface GE_Trading_Sell_ViewController ()<UITableViewDataSource,UITableViewDelegate,ManagerSocketDelegate>

@property (strong, nonatomic) Home_Search_View *searchView;

@property (nonatomic,strong) UITableView *tableView;

@property (nonatomic,strong) GE_Trading_Sell_Cell *cell;

@property (nonatomic,strong) GE_Trading_OneStock_Model *oneStockModel;
...
@property (nonatomic, copy) NSString *sellCodeStr;//股票代码


@end

@implementation GE_Trading_Sell_ViewController
...
- (void)viewDidLoad {
    ...
    self.SellRefreshBlock = ^(NSInteger index) {
        
        [[ManagerSocket sharedManager] closeConnectSocket];
        
        if (weakSelf.sellCodeStr.length > 0) {
            [weakSelf requestSellGetGoodsDetail:weakSelf.sellCodeStr];
        }
        
    };
    
    if (self.sellCodeStr.length > 0) {
        [self requestSellGetGoodsDetail:self.sellCodeStr];
    }
...
-(void)notificationClickedNotifa:(NSNotification *)nitifa
{
    ...
    self.sellCodeStr = [ManagerGlobeUntil sharedManager].goodsCode;
    [self requestSellGetGoodsDetail:[ManagerGlobeUntil sharedManager].goodsCode];
}
    ...
    if (_searchView == nil) {
        ...
        _searchView.SearchBtnBlock = ^(NSString * _Nonnull searchStr) {
            ...
            vc.SearchCodeBlock = ^(NSString * _Nonnull code) {
                weakSelf.sellCodeStr = code;
                [weakSelf requestSellGetGoodsDetail:code];
            };
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.cell)
    {
        self.cell = [[GE_Trading_Sell_Cell alloc]initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:[NSString stringWithFormat:@"chicangView%ld",indexPath.row]];
    }
    ...
    self.cell.SellOrderBlock = ^(NSDictionary * _Nonnull index) {
...
- (void)requestSellGetGoodsDetail:(NSString *)code
{
